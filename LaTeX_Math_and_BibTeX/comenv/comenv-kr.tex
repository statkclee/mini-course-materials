...
\begin{frame}  \frametitle{일반 환경}
일반적으로 환경(environment)은 다음과 같은 형태를 갖는다.
\vspace{0.5mm} \\
\begin{itemize}
...
\end{itemize}
\vspace{0.5mm}
앞에서처럼, {\color{highlight}시작하는 것(begin)} and {\color{highlight}끝나는 것(end)} 에서 인자를 \#1, \dots, \#n 으로 참조한다.
\vspace{3mm} \\
기존 환경을 다시 정의하려면, 위와 동일한 형식으로 {\color{command}$\backslash$renewenvironment} 명령어를 사용한다.
\end{frame}

\begin{frame}  \frametitle{환경 + 계수기}
\begin{itemize}
